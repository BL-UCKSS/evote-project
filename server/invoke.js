...
const peerIdentity = 'User1@org1.example.com';
...
async function queryAll() {
  ...
  try {
    ...
    await gateway.connect(connectionProfile, { wallet, identity: peerIdentity, clientTlsIdentity: peerIdentity, discovery: config.gatewayDiscovery });

    //connect to our channel that has been created on IBM Blockchain Platform
    console.log('get mychannel');
    const network = await gateway.getNetwork('mychannel');
    
    //connect to our insurance contract that has been installed / instantiated on IBM Blockchain Platform
    console.log('get evote');

...
class MyAssetContract extends Contract {
  ...
  async init(ctx) {
    ...
    let voter1 = await new Voter();
    voter1.init('201511454', '1234');
    ...
    if (currElections.length === 0) {
      //(총책01:E01)가 사실 있어야 하는 자리 - 선거 파일이 유효한지 확인 
      //Nov 3 is election day - 예시를 위한 하드코딩이지만 저희도 시작날짜와 종료날짜를 하드코딩해서 제공하면 될거 같습니다.
      let electionStartDate = await new Date(electionData.electionStartYear, electionData.electionStartMonth, electionData.electionStartDay);
      let electionEndDate = await new Date(electionData.electionEndYear, electionData.electionENdMonth, electionData.electionENdDay);
    ...
    for(var prop in ballotData) {
      if (flag){
        temp = ballotData[prop];
        flag = false;
      }
      else {
        let res = await new VotableItem(ctx, temp, prop, ballotData[prop]);
        votableItemsRes.push(res);
        flag = true;
      }
    ...
    // 트랜잭션에 푸쉬
    for (let i = 0; i < votableItemsRes.length; i++) {

      //save votable choices in world state 학생03:B01 - 목록을 업로드하는 역할 원장에서 불러와서 보여주는 것은 프론트에 구현된거 같다
      await ctx.stub.putState(votableItemsRes[i].votableCode, votableItemsRes[i].votableCandidate, Buffer.from(JSON.stringify(votableItemsRes[i])));
    ...
    for (let i = 0; i < voters.length; i++) {
      ...
      if (!voters[i].ballot) {
        ...
        //give each registered voter a ballot - 내부함수를 이용해 투표지 생성 
        await this.generateBallot(ctx, votableItemsRes, election, voters[i]);
   ...
   */
  async generateBallot(ctx, votableItemsRes, election, voter) {

    //generate ballot - 투표지 객체 생성
    let ballot = await new Ballot(ctx, votableItemsRes, election, voter.voterId);
  ...
  async createVoter(ctx, args) {
    ...
    await this.generateBallot(ctx, votableItemsRes, currElection, newVoter);
   ...
   */
  //총책02:B01 - 중단시킬 선거를 선택한다???? - id가 입력으로 들어가기 때문에 이거는 아닌거 같고
...
/* 총책03:실시간 투표율을 확인한다 - 투표결과를 쿼리하는것은 있지만 
투표율은 스마트 컨트렉트에 입력 값이 아니므로 산출할 수 없다.. 
하지만 예시 화면을 보면 서버에 구현될 수 있는 여지가 있다.
...
학생004 - 넘어갔다. 근데 여기서 티안나게 네트워크로 들어오게
하는 부분이 필요할듯


학생03:B01 - 유의사항에 동의한다(미구현-새로운 .js 파일로 목록을 만들어 원장에 추가해줄 수 있음)
, 가능한 선거목록을 확인한다.(구현) - 
학생03:B02 - 프론트에 구현된거 같다.
학생03:A01 - 프론트에 구현된거 같다

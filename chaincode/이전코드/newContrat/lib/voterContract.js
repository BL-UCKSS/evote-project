...
*/
    ...
    for(var prop in ballotData) {
      ...
      let res = await new VotableItem(ctx, ballotData[prop][0], ballotData[prop][1], ballotData[prop][2], "ztue8zxg4z9k64lf60ks2");
...
async modifyElection(ctx, args) {
 ...
 //Nov 3 is election day
   ...
   //make sure we have an election
     ...
     election.check = args.univ + electionStartDate.getFullYear();
...
async createVotableItem(ctx, args) {
  
  args = JSON.parse(args);

  let currElections = JSON.parse(await this.readMyAsset(ctx, 'args.electionId'));
  ...
  //create a new voter
  let votableId = await this.generateVotableItem(ctx ,args.votableId, args.votableCandidate , args.description , args.electionId);

  let response = {};
  response.success = votableId;
  return response;
}


async generateVotableItem(ctx ,votableId, votableCandidate , description , electionId) {

  //generate ballot
  let votableItem = await new VotableItem(votableId, votableCandidate, description, electionId);


  // //update state with ballot object we just created
  await ctx.stub.putState(votableItem.votableId, Buffer.from(JSON.stringify(votableItem)));

  return votableItem.votableId;

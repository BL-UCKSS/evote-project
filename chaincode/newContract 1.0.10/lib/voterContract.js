...
async createCandidateResult(ctx, args) {
 ...
 //선거 시작 전에만 수정할 수 있음 [&& !(await this.validateEndDate(election.endDate))] 제거
  if (await this.validatePreviousDate(election.startDate)){
    //create a new voter
     let candidateId = await this.generateCandidateResult(ctx , args,  args.electionId);
     let response = {};
...
async generateCandidateResult(ctx ,  args , electionId) {
  ...
  //generate ballot
  let candidateResult = await new CandidateResult( args.name, electionId , args.icon ,args.link, args.hakbun1, args.name1 ,
...
/**
 ...
 // student에 vBallot 등록 ->> 왜이렇게 하드코딩했냐면 성능을 위해
  async enrollVBallot(ctx, walletId ,voterId) {
 // student 존재하는지 확인해보고 없으면 초기화
    if(await this.myAssetExists(ctx, walletId)){
      let student = await new VBallot(args.walletId, election);
    ...
    if (student.slot1 === "NULL") {
      ...
      let response = {};
      response.error = '해당 walletId에 slot1의 VBallot이 할당되었습니다.';
      return response;
      ...
      await ctx.stub.putState(student.walletId, Buffer.from(JSON.stringify(student)));
      let response = {};
      response.error = '해당 walletId에 slot2의 VBallot이 할당되었습니다.';
      return response;
      ...
      await ctx.stub.putState(student.walletId, Buffer.from(JSON.stringify(student)));
      let response = {};
      response.error = '해당 walletId에 slot3의 VBallot이 할당되었습니다.';
      return response;
      ...
      await ctx.stub.putState(student.walletId, Buffer.from(JSON.stringify(student)));
      let response = {};
      response.error = '해당 walletId에 slot4의 VBallot이 할당되었습니다.';
      return response;
    ...
    // student 존재하는지 확인해보고 없으면 초기화
      ...
      let queryResults= [];
       ...
       if (student.slot1 !== "NULL") {
        ...
        reVBallot3.election = vBallot3.picked;
        ...
        reVBallot4.election = vBallot4.picked;
...
async queryCurrentTimeTurnout(ctx, args) {
  ...
  if(TotalNum !== 0){
    ...
    let response = {};
    response.Turnout = currentTimeTurnout;
    return response;
} 

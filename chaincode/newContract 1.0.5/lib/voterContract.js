...
async modifyElection(ctx, args) {
 ...
 //Nov 3 is election day
   ...
   //make sure we have an election
     ...
     await electionStartDateValidate.setDate(await date.electionStartDateValidate() - 3);


     if(await this.validateStartDate(electionStartDateValidate)){
      ...
      let response = {};
      response.error = '수정하려는 선거 시작시간이 현재시간 기준으로 3일 이내입니다.';
      return response;
      }
    } else {
      let response = {};
...
async createCandidateResult(ctx, args) {
 ...
 //선거 시작 전에만 수정할 수 있음 [&& !(await this.validateEndDate(election.endDate))] 제거
  if (await this.validateStartDate(election.startDate)){
    //create a new voter
...
/**
 ...
 async validateStartDate(startDate) {
  let currentTime = await new Date();

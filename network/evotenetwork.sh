...
function printHelp() {
  echo "Usage: "
  echo "  evotenetwork.sh <mode> [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-l <language>] [-o <consensus-type>] [-i <imagetag>] [-a] [-n] [-v]"
  ...
  echo "    -l <language> - the chaincode language: node"
  echo "    -o <consensus-type> - the consensus-type of the ordering service: etcdraft"
  echo "    -i <imagetag> - the tag to be used to launch the network (defaults to \"latest\")"
  echo "    -a - launch certificate authorities (no certificate authorities are launched by default)"
  echo "    -n - do not deploy chaincode (abstore chaincode is deployed by default)"
  echo "    -v - verbose mode"
  echo "  evotenetwork.sh -h (print this message)"
  echo
  echo "Typically, one would first generate the required certificates and "
  echo "genesis block, then bring up the network. e.g.:"
  echo
  echo "	evotenetwork.sh generate -c mychannel"
  echo "	evotenetwork.sh up -c mychannel -s couchdb"
  echo "        evotenetwork.sh up -c mychannel -s couchdb -i 1.4.0"
  echo "	evotenetwork.sh up -l node"
  echo "	evotenetwork.sh down -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	evotenetwork.sh generate"
  echo "	evotenetwork.sh up"
  echo "	evotenetwork.sh down"
}
